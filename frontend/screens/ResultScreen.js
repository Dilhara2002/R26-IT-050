...
export default function ResultScreen({ route, navigation }) {
  ...
  const [safetyResult] = useState(null);
  ...
  const busy = Boolean(regenerationLoading) || safetyLoading;
  ...
  const handleRegeneration = async (mode, stop = null) => {
    ...
    const previousState = {
      data, persistence, safetyResult, safetyError, validationError,
      ...
      <View style={styles.guideCard}>
        ...
        {guidePresentation?.structured ? guidePresentation.sections.map((section, index) => (
          ...
          <View style={styles.guideUnavailable}>
            ...
            <Text style={styles.guideUnavailableText}>The optional guide was omitted. Your itinerary, deterministic evidence, map, and Safety handoff are unchanged.</Text>
          </View>
        )}
      </View>

      <View style={styles.safetyCard}>
        <Text style={styles.cardEyebrow}>NEXT STEP · EXISTING COMPONENT</Text>
        <Text style={styles.cardTitle}>Continue to Safety Analysis</Text>
        <Text style={styles.bodyText}>
          The existing Safety Analyzer will receive your starting coordinates and all {optimizedStops.length} ordered stops, including stable place IDs, names, coordinates, and visit durations. Risk evidence may be unavailable or incomplete.
        </Text>
        {availabilityMessage || validationError || safetyError ? (
          <StatusMessage kind="error" text={validationError || safetyError || availabilityMessage} />
        ) : null}
        <ActionButton
          label={safetyLoading ? "Opening Safety Analysis…" : "Continue to Safety Analysis"}
          onPress={handleSafetyAnalysis}
          disabled={busy || Boolean(availabilityMessage)}
...
const styles = StyleSheet.create({
  ...
  safetyButton: { backgroundColor: "#7C3AED", marginTop: 12, marginHorizontal: 0 },
  ...
  safetyCard: { backgroundColor: "#FFFFFF", borderColor: "#C4B5FD", borderWidth: 1, borderRadius: 20, padding: 18 },
