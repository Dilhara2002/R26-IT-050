...
export default function HomeScreen({ navigation }) {
  ...
  const [locationMessage, setLocationMessage] = useState(
    "Checking whether your device location is available…"
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (status !== "granted") {
          setLocationMessage(
            "Location permission was not granted. Choose a point on the map or enter decimal coordinates."
          );
          return;
        }
        ...
        setLocationMessage("Device location detected. Confirm it on the map before generating.");
      } catch {
        if (active) {
          setLocationMessage(
            "Automatic location detection was unavailable. Choose a point on the map or enter decimal coordinates."
          );
        }
      } finally {
  ...
  const mapLatitude = Number.isFinite(parsedLatitude) && parsedLatitude >= -90 && parsedLatitude <= 90
    ? parsedLatitude : MAP_PREVIEW.latitude;
  const mapLongitude = Number.isFinite(parsedLongitude) && parsedLongitude >= -180 && parsedLongitude <= 180
    ? parsedLongitude : MAP_PREVIEW.longitude;
    ...
    <ScrollView style={styles.page} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.card}>
        ...
        <View style={styles.mapContainer}>
          <MapPicker lat={mapLatitude} lon={mapLongitude} onSelect={selectMapLocation} />
          {locationLoading ? (
            <View style={styles.mapStatusOverlay}>
              <ActivityIndicator color="#1D4ED8" />
              <Text style={styles.mapStatusText}>Detecting location…</Text>
            </View>
          ) : null}
        </View>
